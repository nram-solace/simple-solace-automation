...
def main(argv):
    ...
    p.add_argument('--tls', action="store_true", required=False, default=False, 
                   help='Enable TLS config. Default off')
    p.add_argument( '--verbose', '-v', action="count",  required=False, default=0,
                help='Turn Verbose. Use -vvv to be very verbose')
    r = p.parse_args()
    Verbose = r.verbose
    
    targets = ['externalLink', 'remoteAddress', 'vpnDmrBridge']
    if r.tls:
        targets.append('trustedCommonName')
    ...
    print (f"\n===  Create local cluster {cfg_local['dmrClusterName']}")
    post_target ('dmrCluster', cfg_local)
    
    #n += 1
    #cfg_remote = cfg_dmr_remote['cluster']
    #print (f"\n=== Create remote cluster {cfg_remote['dmrClusterName']}")
    #post_target ('dmrCluster', cfg_remote)


    # Process links
    nlinks = len(cfg_dmr_local['links'])
    print (f"\n=== Processing {nlinks} local links")
    i = 0
    for lcfg in cfg_dmr_local['links']:
        i += 1
        print (f"\n--- Processing local link {i}/{nlinks}")
        for target in targets:
            post_target (target, cfg_local, lcfg)
     
    # Create on the remote nodes
    print (f"\n===  Processing {nlinks} remote links")
    i = 0
    for cfg_remote in cfg_dmr_remote['links']:
        i += 1
        print (f"\n---  Processing remote link {i}/{nlinks}")
        post_target ('dmrCluster', cfg_remote)
        for target in (['externalLink', 'vpnDmrBridge']):    
            post_target (target, cfg_remote, cfg_remote)

    # Enable links & cluster
    for lcfg in cfg_dmr_local['links']:
        patch_target ("enableLink", cfg_local, lcfg)
    for cfg_remote in cfg_dmr_remote['links']:
        patch_target ("enableLink", cfg_remote, cfg_remote)

    # Enable clusters - no need to run for cloud instances
    patch_target ("enableCluster", cfg_local)
    for cfg_remote in cfg_dmr_remote['links']:
        patch_target ("enableCluster", cfg_remote, cfg_remote)

def post_target(target, ccfg, lcfg={}):
    cfg = {**ccfg, **lcfg}
    if Verbose > 1:
        print ("   ßpost_target: Config: "); pp.pprint(cfg)
...
def processs_target (target, cfg, action="post"):
    ...
    print (f"*** Working on {end} {target} ({action})")
